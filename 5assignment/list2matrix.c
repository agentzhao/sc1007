...
int main() {
  ...
  int V1, V2;
  printf("Enter two vertices which are adjacent to each other (enter a to "
         "stop):\n");
...
void adjL2adjM(Graph *g) {
  ...
  // allomate mem for matrix and initialize to 0
  g->adj.matrix = malloc(g->V * sizeof(int *));
  for (i = 0; i < g->V; i++) {
    g->adj.matrix[i] = malloc(g->V * sizeof(int));
    for (j = 0; j < g->V; j++)
      g->adj.matrix[i][j] = 0;
  }
