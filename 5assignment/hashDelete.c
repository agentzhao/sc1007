...
int main() {
  ...
  while (opt >= 1 && opt <= 5) {
    ...
    case 1:
      ...
      Q3Hash.Table =
          (HashTableNode *)malloc(sizeof(HashTableNode) * (Q3Hash.hSize));
...
int HashInsert(HashTable *Q3HashPtr, int key) {
  ...
  // The key is inserted from the front. It is not the same approach discussed
  // in lecture
...
int HashDelete(HashTable *Q3HashPtr, int key) {
  ...
  temp = Q3HashPtr->Table[index].head; // first node
  prev = NULL;

  while (temp != NULL) {
    if (temp->key == key) {
      if (prev == NULL) {
        // go to next hash value
