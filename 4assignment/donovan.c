...

{
  ...
  while (t == 0 && !isEmptyStack(s)) {
    if (curr->left != NULL) {
      push(&s, curr);
      curr = curr->left;
    }
    // enqueue downwards naturally if only right exists
    ...
    // endpoint
      ...
      if ((isEmptyStack(s) == 1) && (curr->left == NULL) &&
          (curr->right == NULL)) {
        t = 1;
        break;
      }
      if ((isEmptyStack(s) != 1) && (peek(s)->right != NULL)) {
        enqueue(q, peek(s));
        curr = peek(s)->right;
        if (!isEmptyStack(s)) {
          pop(&s);
  ...
  // reduce get the correct order
  while (isEmptyQueue(*q) != 1) {
    temp = getFront(*q);
    dequeue(q);
    // right>left, temp is left
    // if right < left;
    if (getFront(*q)->item < temp->item) {
      // search queue until the swap is found
      while (getFront(*q)->item < temp->item) {
        temp2 = getFront(*q);
        dequeue(q);
      }
      a = temp2->item;
