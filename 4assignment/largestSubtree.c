...
// solution
int isBST(BTNode *root, BTNode *l, BTNode *r) {
  // Base condition
  if (root == NULL)
    return 1;
...
BTNode *compareBST(BTNode *first, BTNode *second) {
  if (isBST(first, NULL, NULL) && isBST(second, NULL, NULL)) {
    if (countNodes(first) >= countNodes(second))
      return first;
    else
      return second;
  } else if (isBST(first, NULL, NULL)) {
    return first;
  } else if (isBST(second, NULL, NULL)) {
    return second;
  } else {
    return NULL;
  }
...
BTNode *findLargestBST(BTNode *root) {
  ...
  return compareBST(findLargestBST(root->left), findLargestBST(root->right));
}
